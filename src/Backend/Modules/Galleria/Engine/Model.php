<?php
namespace Backend\Modules\Galleria\Engine;
...
class Model
{
	...
	public static function insertAlbum(array $item)
	{
		...
		$extra = array('module' => 'Galleria', 'type' => 'widget', 'label' => 'Galleria', 'action' => 'Slideshow', 'data' => serialize(array('id' => $item['id'], 'extra_label' => "Slideshow Gallery  " . $item['title'], 'language' => $item['language'], 'edit_url' => BackendModel::createURLForAction('edit_album') . '&id=' . $item['id'])), 'hidden' => 'N', 'sequence' => $db->getVar('SELECT MAX(i.sequence) + 1
				 FROM modules_extras AS i
				 WHERE i.module = ?', array('links')));
	...
	public static function updateImage(array $item)
	{
		...
		// update the category
		$update = $db->update('galleria_images', $item, 'id = ?', array($item['id']));
	}
	/**
	 * Update an image
	 *
